...
import static web.app.util.constants.UserRole.ROLE_USER;
...
//            userService.save(Role.builder().name(ROLE_SUPER_USER.getValue()).build());
            ...
            userService.signUp(SignUpDto.builder().name("King").email("kingl@mail.ru").password("king").build());

            userService.addAuthorityToUser("kingl@mail.ru", ROLE_USER.getValue());
        };
    }
